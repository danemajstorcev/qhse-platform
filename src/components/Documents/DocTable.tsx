...
export default function DocTable({ documents, onBumpVersion, onChangeStatus, onView }: Props) {
        ...
        <tbody>
          {documents.map(doc => (
            <tr key={doc.id}>
              ...
              <td>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div className="doc-icon">
                    <FileText
                      size={15}
                      style={{ color: doc.fileType ? FILE_TYPE_COLOR[doc.fileType] : 'var(--text-muted)' }}
                    />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div className="cell-primary" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 200 }}>
                      {doc.title}
                    </div>
                    {doc.fileType && (
                      <div style={{ fontSize: 10, color: FILE_TYPE_COLOR[doc.fileType], fontWeight: 700, marginTop: 1 }}>
              ...
              <td style={{ fontSize: 12, whiteSpace: 'nowrap' }}>{doc.owner}</td>
              <td style={{ fontSize: 12, fontFamily: 'JetBrains Mono, monospace', whiteSpace: 'nowrap' }}>{doc.lastModified}</td>
