...
import { ChevronDown, ChevronUp, Trash2, Plus, CheckCircle } from 'lucide-react'
...
export default function AuditRow({
  audit, expanded, onToggle, onStatusChange, onDelete, onAddFinding, onCloseFinding,
}: Props) {
        ...
        {expanded && (
          <div style={{ paddingBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16, lineHeight: 1.6 }}>
              {audit.scope || <span style={{ fontStyle: 'italic', color: 'var(--text-muted)' }}>No scope defined.</span>}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.8 }}>
                Findings {audit.findings.length > 0 && `(${audit.findings.length})`}
              </div>
              <button
                className="btn btn-sm btn-ghost"
                onClick={e => { e.stopPropagation(); setShowFindingModal(true) }}
            ...
            {audit.findings.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {audit.findings.map(f => (
                  <FindingCard key={f.id} finding={f} onClose={() => onCloseFinding(f.id)} />
                ))}
              </div>
            ) : (
              <div style={{
...
function FindingCard({ finding, onClose }: { finding: AuditFinding; onClose: () => void }) {
  return (
    <div style={{
      display: 'flex',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
        <span className={`badge ${finding.status === 'Open' ? 'badge-open' : 'badge-closed'}`}>
          {finding.status}
        </span>
        {finding.status === 'Open' && (
          <button
            className="btn btn-sm btn-ghost"
            onClick={onClose}
            style={{ color: 'var(--emerald)', borderColor: 'var(--emerald-dim)', display: 'flex', alignItems: 'center', gap: 4 }}
          >
