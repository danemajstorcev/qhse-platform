...
export default function IncidentTracking({ incidents, onIncidentsChange }: Props) {
  ...
  const handleAdd = (data: Omit<Incident, 'id'>) => {
    ...
    toast(`Incident reported: ${data.title}`, data.severity === 'Critical' ? 'error' : 'success')
  }

  const handleStatusChange = (id: string, status: IncidentStatus) => {
    onIncidentsChange(incidents.map(i => i.id === id ? { ...i, status } : i))
    toast(`Incident status updated to ${status}`, status === 'Closed' || status === 'Resolved' ? 'success' : 'info')
  }
  ...
  const handleExport = () => {
    const headers = ['ID', 'Title', 'Date', 'Location', 'Severity', 'Status', 'Reporter', 'Description']
    const rows = incidents.map(i => [i.id, i.title, i.date, i.location, i.severity, i.status, i.reporter, i.description])
