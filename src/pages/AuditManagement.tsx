...
export default function AuditManagement() {
  ...
  const handleAddFinding = (auditId: string, finding: Omit<AuditFinding, 'id'>) => {
  ...
  const handleCloseFinding = (auditId: string, findingId: string) => {
    setAudits(prev => prev.map(a => {
      ...
      return {
        ...a,
        findings: a.findings.map(f => f.id === findingId ? { ...f, status: 'Closed' as const } : f),
      }
    }))
  ...
  const handleExport = () => {
    const headers = ['ID', 'Title', 'Standard', 'Type', 'Auditor', 'Date', 'Status', 'Scope', 'Total Findings', 'Open Findings']
    const rows = audits.map(a => [
      a.id, a.title, a.standard, a.type, a.auditor,
      a.scheduledDate, a.status, a.scope,
      a.findings.length,
    ...
    <div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{filtered.length} audits</span>
        </div>
        ...
        <div>
          {filtered.map(audit => (
            <AuditRow
              ...
              onCloseFinding={findingId => handleCloseFinding(audit.id, findingId)}
            />
          ))}
          {filtered.length === 0 && (
