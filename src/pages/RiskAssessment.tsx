...
export default function RiskAssessment({ risks, onRisksChange }: Props) {
  ...
  const handleAdd = (riskData: Omit<Risk, 'id'>) => {
    ...
    toast(`Risk "${riskData.title}" added as ${riskData.level}`, riskData.level === 'Critical' ? 'error' : riskData.level === 'High' ? 'warning' : 'success')
  }
  ...
  const handleExport = () => {
    const headers = ['ID', 'Title', 'Department', 'Likelihood', 'Severity', 'Score', 'Level', 'Standard', 'Owner', 'Controls', 'Review Date']
    const rows = risks.map(r => [r.id, r.title, r.department, r.likelihood, r.severity, r.score, r.level, r.standard, r.owner, r.controls, r.reviewDate])
