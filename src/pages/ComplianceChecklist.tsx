...
export default function ComplianceChecklist() {
  ...
  const handleExport = () => {
    const allItems = STANDARDS.flatMap(s =>
      (items[s] || []).map((i: { clause: string; requirement: string; status: string; evidence?: string; lastAudited?: string }) => [
        s, i.clause, i.requirement, i.status, i.evidence ?? '', i.lastAudited ?? '',
      ])
    )
    downloadCSV(
      [['Standard', 'Clause', 'Requirement', 'Status', 'Evidence', 'Last Audited'], ...allItems],
